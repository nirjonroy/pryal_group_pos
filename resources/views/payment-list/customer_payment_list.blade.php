...
<div class="container-fluid flex-grow-1 container-p-y" id="print">
    ...
    <h4 class="font-weight-bold py-3 mb-0">Customer Payment List</h4>
            ...
            <div class="card-header no-print">
                <form>
                    <div class="row">
                        ...
                        <div class="form-group col-md-1">
                            <br><br><a class="btn btn-info btn-sm" href="{{ action('PaymentController@customerPaymentList')}}">Refresh</a>
                            ...
                            <tr>
                                ...
                                <td class="no-print">
                                    ...
                                    @if(!auth()->user()->hasRole('admin') and ( date('Y-m-d') == date('Y-m-d', strtotime($item->created_at))))
                                       ...
                                       <a class="btn btn-success btn-sm" href="{{ action('PaymentController@customerPaymentEdit',$item->id)}}"><i class="fa fa-edit"></i></a>
                                    @endif
                                    @if(auth()->user()->hasRole('admin'))
                                      <a class="btn btn-success btn-sm" href="{{ action('PaymentController@customerPaymentEdit',$item->id)}}"><i class="fa fa-edit"></i></a>
                                    @endif
                                    <a class="btn btn-primary btn-sm" href="{{ action('PaymentController@customerPaymentDetails',$item->id)}}"><i class="fa fa-eye"></i></a>
                                     @if(auth()->user()->hasRole('admin'))
                                    <a class="btn btn-danger btn-sm" 
                                    href="{{ action('PaymentController@customerPaymentDelete',$item->id)}}" onclick="return confirm(' you want to delete?');">
