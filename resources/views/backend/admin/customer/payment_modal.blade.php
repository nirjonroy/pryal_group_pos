...
$due=($customer->sell->sum('total_price')- $customer->Sell_payment->sum('total_price'));
@endphp
...
<div class="modal-dialog modal-lg" role="document">
  <div class="modal-content">
    ...
    <div class="modal-body">
      <form method="POST" action="{{ action('CustomerController@purchasePayment') }}">

        @csrf
                <div class="modal-header">
                    <h5 class="modal-title">customer
                    <span class="font-weight-light" style="margin-right:5px; ">Name</span>
                    #<span id="invoice_no"> {{$customer->name}}</span>
                    <br>
                ...
                <div class="modal-body">
                    <div class="form-row" style="margin-bottom: 5%;">
                        <div class="form-group col mb-0">
                            ...
                            <stron id="total_amount" style="color:blue;">{{$customer->sell->sum('total_price') }}</stron>
                 
                            <div class="clearfix"></div>
                        </div>
                        <div class="form-group col mb-0">
                            <label class="form-label" style="color:green;">Total Paid Amount</label><br/>
                            <strong id="paid_amount" style="color:green;">{{$customer->sell_payment->sum('total_price')}}</strong>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col mb-0">
                            ...
                            <input name="payment_amount" type="number" step="any" id="payment_amount" min="1" max="{{$due}}" autofocus class="form-control" placeholder="Payment Amount" required value="{{$due}}">
                            <div class="clearfix"></div>
                        </div>
                    </div>
