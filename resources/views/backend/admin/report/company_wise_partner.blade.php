...
<div class="container-fluid flex-grow-1"  id="print">
    <h4 class="font-weight-bold py-3 mb-0">Company Wise Partner Investment Project Report</h4>
                ...
                <div class="card-datatable table-responsive">
                    <table class="datatables-demo table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            @php
                                $project +=$company->projects_p->count();
                                $project_value +=$company->projects_p->sum('project_value');
                                $purchase +=$company->purchase_p->sum('total_price');
                                $receive +=$company->payments_p->sum('payment_amount');
                                $expense +=$company->expense_p->sum('total_price');
                                $due =$company->payments_p->sum('payment_amount') -($company->purchase_p->sum('total_price') +$company->expense_p->sum('total_price'));

                                $total_due +=$due;

                                if($company->projects_p->count() <1){
                                    continue;
                                }
                            @endphp
                            <tr>
                                ...
                                <td>{{number_format($company->projects_p->count(),2)}}</td>
                                <td>{{number_format($company->projects_p->sum('project_value'),2)}}</td>
                                <td>{{number_format($company->payments_p->sum('payment_amount'),2)}}</td>
                                <td>{{number_format($company->purchase_p->sum('total_price'),2)}}</td>
                                <td>{{number_format($company->expense_p->sum('total_price'),2)}}</td>
                                ...
                                <td class="no-print"><a class="btn btn-info btn-sm" href="{{action('Backend\ReportController@companyDetails',$company->id)}}">Details</a></td>
                            </tr>
                            @endforeach
                            <tr>
