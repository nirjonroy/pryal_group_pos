...
<!--########################################################################-->
    ...
    <h4 class="font-weight-bold py-3 mb-0">Supplier List</h4>
    <div class="text-muted small mt-0 mb-4 d-block breadcrumb no-print">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('home') }}"><i class="feather icon-home"></i></a></li>
            <li class="breadcrumb-item"><a href="{{ route('admin.supplier.create') }}">Supplier Create</a></li>
        </ol>
    </div>
                    ...
                    <table class="datatables-demo table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$item->stock_purchase->sum('total_price')}}</td>
                                <td>{{$item->purchaseStockpayment->sum('total_price')}}</td>
                                <td>{{$item->stock_purchase->sum('total_price')-$item->purchaseStockpayment->sum('total_price')}}</td>
                                <td style="width:10%;" class="no-print">
                                    @if(auth()->user()->can('supplier.delete'))
                                     <a onclick="return confirm('Are you sure?')" href="{{ route('admin.supplier.delete',$item->id) }}" class="btn btn-sm btn-danger">Delete</a>
                                    @endif
                                    <a href="{{ route('admin.supplier.show',$item->id) }}" class="btn btn-sm btn-success">View</a>
                                    
                                     @if(!auth()->user()->hasRole('admin') and ( date('Y-m-d') == date('Y-m-d', strtotime($item->created_at))))
                                        <a class="dropdown-item" href="{{ route('admin.supplier.edit',$item->id) }}">Edit</a>
                                    @endif
                                    @if(auth()->user()->hasRole('admin'))
                                     <a class="btn btn-sm btn-success" href="{{ route('admin.supplier.edit',$item->id) }}">Edit</a>
                                     
                                    @endif
                                    @if(auth()->user()->can('supplier.payment'))
                                        @if($item->stock_purchase->sum('total_price') > $item->purchaseStockpayment->sum('total_price') )
                                        <a class="btn btn-sm btn-warning btn_modal" data-href="{{ route('admin.getSupplierPaymentModal',[$item->id]) }}">Add Payment</a>
                                        @endif
                                    @endif
