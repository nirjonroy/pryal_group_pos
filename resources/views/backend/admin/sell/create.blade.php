...
<!--########################################################################-->
    ...
    <h4 class="font-weight-bold py-3 mb-0">Create Sell</h4>
...
<form action="{{ route('store.sell') }}" method="POST">
    @csrf
    <div class="row">
            ...
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <select name="customer_id"  class="form-control select2" required>
                        ...
                        <option {{ old('customer_id') == $item->id ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                  ...
                  <div class="row" style="margin-bottom: 5%;margin-top:10%;">
                    ...
                    <input id="submit" type="submit"  class="btn btn-info pull-right" value="Create Transfer">
                </div>
...
<div id="ProductDefaultLoading" data-url="{{route('ProductDefaultLoading')}}"></div>
<div id="sellUpdateQtyPrice" data-url="{{route('sellUpdateQtyPrice')}}"></div>
@push('js')
...
<script>
    $(document).ready(function(){
        ...
        //default loading
        let url = $('#ProductDefaultLoading').data('url');
        $.ajax({
            url:url,
            type:'GET',
            datatype:'html',
...
<script>
        ...
        let url = $('#sellUpdateQtyPrice').data('url');
                $.ajax({
                    url:url,
                    type:'GET',
                    datatype:'html',
...
<script type="text/javascript">
    ...
    $('select[name="customer_id"]').on('change click', function() {
        $('#form_section').html('');
