...
@section('page_title') | Sell Update @endsection
@push('css')
<style>
    .red{color:red;}
    .gray{color:gray;}
...
<!--########################################################################-->
    ...
    <h4 class="font-weight-bold py-3 mb-0">Update sell</h4>
            ...
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <select name="store_id"  class="form-control" required>
                        ...
                        <option value="{{$store->id}}" {{ $sell->store_id == $store->id ? 'selected' : 'disabled' }}>{{$store->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="card-body">
                    <table class="datatables-demo table table-striped table-bordered">
                        ...
                        @foreach ($sell->sellDetails as $key => $item)
                                @php
                                    $available_qty=($item->quantity+getPurchaseStock($item->product_id,$sell->store_id));
                                @endphp
                                <tr>
                   ...
                   <div> 
                    ...
                    <div class="row" style="margin-bottom: 5%;margin-top:10%;">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" step="any" name="payment_amount" id="paymentAmount" class="form-control"  value="">
                        </div>
                    </div>
...
<div id="getUrl" data-url="{{route('addProductsell')}}"></div>
<div id="addToCartProductDefaultLoading" data-url="{{route('ProductDefaultLoading')}}"></div>
<div id="addToCartProductUpdateQtyPrice" data-url="{{route('sellUpdateQtyPrice')}}"></div>
<div id="addToCartProductUpdateQtyPrice" data-url="{{route('sellUpdateQtyPrice')}}"></div>
<div id="addToCartProductUpdateQtyPrice" data-url="{{route('sellUpdateQtyPrice')}}"></div>
@push('js')
<script>
...
<script>
        ...
        $(document).on('blur','.sale_unit_price', function(ee){
        ee.preventDefault();
...
<script>
    ...
    $(document).on('change','.clickToGet', function(eve){
        var current_value=Number($(this).val());
        var available=Number($(this).data('qty'));
